# phs ↔ pxi-init 비교 (v1.9.11 기준)

pxi-init은 dalsoop의 내부 도구 **phs (proxmox-host-setup)**에서 공유 가치
...
**pxi-init은 phs의 범용 가치 있는 기능을 모두 이식하고, 안전성/테스트/제거 경로에서 크게 개선한 OSS 프로젝트**입니다. dalsoop 특화 운영 기능(OpenClaw, homelable, rbac 등)만 제외하고 나머지는 전부 이식 완료.
