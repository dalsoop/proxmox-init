...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Stop { vmid } => {
            common::run(
                "qm",
                &["shutdown", &vmid, "--timeout", "60", "--forceStop", "1"],
            )?;
            println!("✓ VM {vmid} 정지");
            Ok(())
        }
        ...
        Cmd::Setup {
            ...
            user,
            password,
        } => vm_setup(&vmid, &ip, &user, password.as_deref()),
        ...
        Cmd::Clipboard {
            ...
            user,
            spice,
        } => vm_clipboard(&vmid, &ip, &user, spice),
        Cmd::Type { vmid, text, enter } => {
            vm_type(&vmid, &text, enter);
            Ok(())
        }
    }
}
...
fn ensure_vm_running(vmid: &str) -> anyhow::Result<()> {
    ...
    if status.is_empty() {
        anyhow::bail!("VMID {vmid}를 찾을 수 없습니다");
    }
    if !status.contains("running") {
        anyhow::bail!("VM {vmid}이 실행 중이 아닙니다 (현재: {status})");
    }
    Ok(())
}
...
fn vm_setup(vmid: &str, ip: &str, user: &str, password: Option<&str>) -> anyhow::Result<()> {
    ...
    let pubkey = if std::path::Path::new(pubkey_path).exists() {
        ...
        let output = Command::new("ssh-keygen")
            .args(["-t", "ed25519", "-f", "/root/.ssh/id_ed25519", "-N", ""])
            .output()?;
        if !output.status.success() {
            anyhow::bail!("SSH 키 생성 실패");
        }
    ...
    if vm_ssh_test(ip, user) {
        ...
        if let Some(pw) = password {
            ...
            let status = Command::new("ssh-copy-id")
                ...
                .status()?;
            if !status.success() {
                anyhow::bail!("ssh-copy-id 실패");
            }
        }
        if !vm_ssh_test(ip, user) {
            anyhow::bail!("SSH 키 접속 여전히 실패");
        }
    ...
    if installed {
        ...
        if ok {
            println!("[vm-setup] qemu-guest-agent 설치 완료");
        } else {
            anyhow::bail!("qemu-guest-agent 설치 실패: {out}");
        }
    }
    let _ = vm_exec(
        ip,
    ...
    if !config.contains("agent:") || config.contains("agent: 0") {
        let _ = Command::new("qm")
            .args(["set", vmid, "--agent", "enabled=1"])
            .output();
    }

    // guest agent ping
    let ga = Command::new("qm")
        .args(["guest", "cmd", vmid, "ping"])
        .output();
    match ga {
...
fn vm_korean(vmid: &str, ip: &str, user: &str) -> anyhow::Result<()> {
    ...
    let (has_locale, _) = vm_exec(
        ip,
        user,
        &["bash", "-c", "locale -a 2>/dev/null | grep -q ko_KR.utf8"],
    );
    if !has_locale {
        let _ = vm_exec(
            ip,
            user,
            &[&format!(
                "{sudo}sed -i 's/# ko_KR.UTF-8/ko_KR.UTF-8/' /etc/locale.gen && {sudo}locale-gen"
            )],
        );
    }
    let (lang_ok, _) = vm_exec(
        ip,
        user,
        &[
            "bash",
            "-c",
            "grep -q 'LANG=.*ko_KR' /etc/default/locale 2>/dev/null",
        ],
    );
    if !lang_ok {
        let _ = vm_exec(
            ...
            user,
            &[&format!(
                "{sudo}bash -c \"echo 'LANG=ko_KR.UTF-8' > /etc/default/locale\""
            )],
        );
    }
    ...
    if !need_ibus.is_empty() {
        ...
        let _ = vm_exec(
            ...
            user,
            &[&format!(
                "{sudo}DEBIAN_FRONTEND=noninteractive apt-get install -y -qq {pkg_list}"
            )],
        );
    }
    ...
    // ibus 환경변수
    let (has_ibus_env, _) = vm_exec(
        ip,
        user,
        &[
            "bash",
            "-c",
            "grep -q GTK_IM_MODULE=ibus /etc/environment 2>/dev/null",
        ],
    );
    if !has_ibus_env {
        ...
        let _ = vm_exec(
            ...
            user,
            &[&format!(
                "{sudo}bash -c \"cat >> /etc/environment << 'IEOF'\n{ibus_env}\nIEOF\""
            )],
        );
    }
    ...
    let (tz_ok, _) = vm_exec(
        ...
        &[
            "bash",
            "-c",
            "timedatectl show --value -p Timezone 2>/dev/null | grep -q Asia/Seoul",
        ],
    );
    if !tz_ok {
        let _ = vm_exec(
            ip,
            user,
            &[&format!("{sudo}timedatectl set-timezone Asia/Seoul")],
        );
    }
...
fn vm_clipboard(vmid: &str, ip: &str, user: &str, spice: bool) -> anyhow::Result<()> {
    ...
    if !need.is_empty() {
        ...
        if !ok {
            eprintln!("[vm-clipboard] 패키지 설치 실패: {out}");
        }
    }
    ...
    if spice {
        ...
        if !config
            .lines()
            .any(|l| l.starts_with("vga:") && l.contains("qxl"))
        {
            let _ = Command::new("qm")
                .args(["set", vmid, "--vga", "qxl"])
                .output();
...
fn char_to_keyname(c: char) -> Option<String> {
    match c {
        ...
        ' ' => Some("spc".into()),
        '-' => Some("minus".into()),
        '_' => Some("shift-minus".into()),
        '=' => Some("equal".into()),
        '+' => Some("shift-equal".into()),
        '.' => Some("dot".into()),
        ',' => Some("comma".into()),
        '/' => Some("slash".into()),
        '\\' => Some("backslash".into()),
        '\'' => Some("apostrophe".into()),
        '"' => Some("shift-apostrophe".into()),
        ';' => Some("semicolon".into()),
        ':' => Some("shift-semicolon".into()),
        '[' => Some("bracket_left".into()),
        ']' => Some("bracket_right".into()),
        '`' => Some("grave_accent".into()),
        '~' => Some("shift-grave_accent".into()),
        '!' => Some("shift-1".into()),
        '@' => Some("shift-2".into()),
        '#' => Some("shift-3".into()),
        '$' => Some("shift-4".into()),
...
mod tests {
    ...
    fn list_only_header_returns_empty() {
        assert!(parse_qm_list("VMID NAME STATUS MEM DISK PID")
            .unwrap()
            .is_empty());
    }
