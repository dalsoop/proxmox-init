...
fn require_installed() {
    if !Path::new(COMPOSE_FILE).exists() {
        eprintln!("Infisical이 설치되어 있지 않습니다. `prelik run infisical install`을 먼저 실행하세요.");
        std::process::exit(1);
    }
}
...
fn infisical_install(port: u16) {
    ...
    if Path::new(COMPOSE_FILE).exists() {
        ...
        println!("재설치하려면 먼저 `prelik run infisical uninstall --force`를 실행하세요.");
        return;
    }
    ...
    let env_content = format!(
        r#"# Infisical configuration (auto-generated by prelik)
...
"#
    ...
    if common::run("docker", &["compose", "-f", COMPOSE_FILE, "up", "-d"]).is_ok() {
        ...
        eprintln!("컨테이너 시작 실패. `prelik run infisical logs --tail 50`으로 확인하세요.");
    }
}
...
fn infisical_uninstall(force: bool) {
    ...
    if !force {
        ...
        eprintln!("  prelik run infisical uninstall --force");
        std::process::exit(1);
    }
