...
fn list(json: bool) -> anyhow::Result<()> {
    ...
    println!(
        "  {:<20} {:<10} {:<6} {:<10} {}",
        "NODE", "STATUS", "CPUS", "MEM(GB)", "UPTIME"
    );
    for r in &rows {
        println!(
            "  {:<20} {:<10} {:<6} {:<10} {}d",
...
fn count_running(json_text: &str) -> (usize, usize) {
    ...
    let running = arr
        .iter()
        .filter(|x| x["status"].as_str() == Some("running"))
        .count();
    (total, running)
}
...
fn exec(node: &str, cmd: &[String]) -> anyhow::Result<()> {
    ...
    let status = Command::new("ssh")
        .args([
            "-F",
            "/dev/null", // user/system ssh_config 무시
            "-o",
            "ConnectTimeout=10",
            "-o",
...
fn node_join(node_name: &str, ssh_target: Option<&str>) -> anyhow::Result<()> {
    ...
    // 클러스터에 이미 있는지 확인
    let nodes_json = common::run("pvesh", &["get", "/nodes", "--output-format", "json"])?;
    let exists = serde_json::from_str::<serde_json::Value>(&nodes_json)
        .ok()
        .and_then(|v| {
            v.as_array()
                .map(|arr| arr.iter().any(|n| n["node"].as_str() == Some(node_name)))
        })
        .unwrap_or(false);
    ...
    if std::path::Path::new(binary_path).exists() {
        let scp_ok = Command::new("scp")
            .args([
                ...
                "StrictHostKeyChecking=no",
                binary_path,
                &format!("root@{node_ip}:/tmp/phs-new"),
            ])
            .status()
            .map_or(false, |s| s.success());
        if scp_ok {
            let mv_ok = ssh_run(
                &node_ip,
                "chmod 755 /tmp/phs-new && mv /tmp/phs-new /usr/local/bin/proxmox-host-setup",
            );
            println!(
                "     {}",
                if mv_ok {
...
fn node_remove(node_name: &str, force: bool) -> anyhow::Result<()> {
    ...
    if !common::has_cmd("pvecm") {
        anyhow::bail!("pvecm 없음");
    }
    ...
    let running = lxcs
        .iter()
        .filter(|l| l["status"].as_str() == Some("running"))
        .count();

    if !lxcs.is_empty() && !force {
        anyhow::bail!(
            "LXC {}개 남아있음 (실행 중 {running}). --force 필요",
            lxcs.len()
        );
    }
    if running > 0 {
    ...
    let result = Command::new("pvecm")
        .args(["delnode", node_name])
        .output()?;
...
fn node_sync(node_name: &str) -> anyhow::Result<()> {
    ...
    let local_hash = local_hash
        ...
        .unwrap_or("")
        .to_string();
    let remote_hash = ssh_capture(
        &node_ip,
        &format!("sha256sum {LOCAL_BIN} 2>/dev/null || true"),
    );
    ...
    let exists = ssh_capture(
        &node_ip,
        &format!("test -f {CONFIG_FILE} && echo yes || echo no"),
    );
    if exists.trim() == "yes" {
        ...
        let list = ssh_capture(
            &node_ip,
            "pct list 2>/dev/null | awk 'NR>1 {print $1}' | head -10",
        );
        ...
        for vmid in list.lines() {
            ...
            let cfg = ssh_capture(
                &node_ip,
                &format!("pct config {vmid} 2>/dev/null | grep '^net0:' | head -1"),
            );
            if !cfg.contains("bridge=vmbr1") {
                continue;
            }
            for field in cfg.split(',') {
                let field = field.trim();
                if let Some(v) = field.strip_prefix("gw=") {
                    gateway = v.to_string();
                }
        ...
        // fallback: vmbr1 IP
        if gateway.is_empty() {
            let cidr = ssh_capture(
                &node_ip,
                "ip -4 addr show vmbr1 2>/dev/null | awk '/inet / {print $2; exit}'",
            );
            let cidr = cidr.trim();
        ...
        if !gateway.is_empty() {
            ...
            let write_cmd = format!(
                "mkdir -p /etc/proxmox-host-setup && cat > {CONFIG_FILE} <<'EOF'\n{config_body}EOF"
            );
